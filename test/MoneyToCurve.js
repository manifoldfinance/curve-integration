...
contract("Integrate Curve.Fi into your defi", async ([owner, defiowner, user1, user2]) => {
  ...
  describe("Deposit your money into Curve.Fi", () => {
    it("Deposit", async () => {
      ...
      await moneyToCurve.multiStepDeposit([deposits.dai, deposits.usdc, deposits.tusd, deposits.usdt], { from: user1 });
    ...
    it("Y-tokens are deposited to Curve.Fi Swap", async () => {
      expect((await ydai.balanceOf(curveSwap.address)).toString(), "YDAI not deposited").to.equal(
        deposits.dai.toString(),
      );
      expect((await yusdc.balanceOf(curveSwap.address)).toString(), "YUSDC not deposited").to.equal(
        deposits.usdc.toString(),
      );
      expect((await ytusd.balanceOf(curveSwap.address)).toString(), "YTUSD not deposited").to.equal(
        deposits.tusd.toString(),
      );
      expect((await yusdt.balanceOf(curveSwap.address)).toString(), "YUSDT not deposited").to.equal(
        deposits.usdt.toString(),
      );
    });
    ...
    it("Curve.Fi LP-tokens are staked in Gauge", async () => {
      ...
      expect((await curveGauge.balanceOf(moneyToCurve.address)).toString(), "Stake is absent in Gauge").to.equal(
        lptokens.toString(),
      );
    });
  ...
  describe("Additional deposit to create extra liquidity", () => {
    it("Additional deposit", async () => {
      ...
      await moneyToCurve.multiStepDeposit([deposits.dai, deposits.usdc, deposits.tusd, deposits.usdt], { from: user2 });
    });
  });
